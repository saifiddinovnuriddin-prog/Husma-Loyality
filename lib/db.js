...
function normalizePhone(phone) {
  ...
  if (!digits) return "";
  // Возвращаем формат с плюсом +998...
...
export async function getUserByPhone(phone) {
  const formattedPhone = normalizePhone(phone); // "+998901234567"
  const rawDigits = formattedPhone.replace("+", ""); // "998901234567"

  // Ищем и по формату с +, и без + для надежности
...
export async function createUser({ name, country, phone, password }) {
  ...
  let attempts = 0;
  while (attempts < 10) {
    cardNumber = generateUniqueCardNumber(() => false);
